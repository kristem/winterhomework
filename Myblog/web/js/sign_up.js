...
function clearAll() {
	if(name_able) {
		$(".nick_name_box input").className = "normal";
	}
 ...
 */
function changeMag (str, pos, isClr) {
	var box = null;
	var p = null;
	isClr = isClr || false;
	if(pos === 1) {
		box = $(".nick_name_box input");
		p = $(".nick_name_box .mas_p");
	}
	else if(pos === 2) {
...
function check (uName, email, uPsw, uCode) {
	clearAll();
	if(uName === "") {
		changeMag("昵称不得为空!", 1);
		return false;
		...
		changeMag("昵称最小为2位字符!", 1);
		return false;
	}
	else if(uName.length > 18) {
		changeMag("昵称长度不超过18字符!", 1);
		return false;
	}
	else if(!/^([\u4e00-\u9fa5]|\w)+$/.test(uName)) {
		changeMag("昵称包含非法字符!", 1);
		return false;
	}
	else if(uPsw === "") {
		changeMag("密码不得为空!", 2);
	...
	else if(uPsw.length < 7) {
		changeMag("密码长度最小为6位!", 2);
		return false;
	}
	else if(uPsw.length > 16) {
		changeMag("密码长度不超过16字符!", 2);
		return false
	}
	else if(!/[A-Za-z]/.test(uPsw)){
		changeMag("密码至少含有一位字母!", 2);
		return false;
	}
	else if(email === "") {
		changeMag("邮箱不得为空!", 3);
	...
	if(check(uName, uEmail, uPsw, uCode)){
		ajax({
			...
			success: function(msg){
				...
				else if(jsonObj.result === 'userExist'){
					clearAll();
					changeMag('昵称已被注册!', 1);
				}
				else {
...
document.addEventListener("DOMContentLoaded", function(event){
	...
	inputs[0].addEventListener("input", function(){
		...
		ajax({
			...
			method: "GET",
			success: function(msg) {
				if(JSON.parse(msg).result === "userExist") {
					name_able = 0;
					changeMag('该昵称已被占用', 1);
				}
				else {
	...
	checkBox.addEventListener("click", function(){
		var bnt = $("#submit_bnt");
		if(bnt.className === 'is_agree') {
			bnt.className = "un_agree";
			bnt.removeEventListener("click", doSubmit);
		}
		else {
